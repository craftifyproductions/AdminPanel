-- =============================================================================
-- Craftify AI Admin Panel · Activity logs
-- =============================================================================
-- Run this in Supabase Dashboard → SQL Editor (paste all, then Run).
--
-- The Next.js Craftify AI Admin Panel writes/reads rows with the SERVICE ROLE key
-- (server-side only). Service role bypasses RLS; RLS is still enabled so the
-- table is not world-readable via the anon key.
--
-- Optional: authenticated Supabase users may SELECT their own rows.
-- =============================================================================
...
-- Authenticated users can read their own rows (optional; the app uses service role for MVP).
...
-- Revoke broad grants from anon; keep authenticated SELECT via policy above.
